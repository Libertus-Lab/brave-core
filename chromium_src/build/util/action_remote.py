...
# Add PYTHONPATH and PATH env vars to remote execution. Convert
# exec_root-included paths to relative, keep system paths absolute.
def AddRelativePythonPathsToRbe(kwargs):
    ...
    for p in os.environ.get("PYTHONPATH", "").split(os.pathsep):
        try:
            if os.path.isabs(p) and p.startswith(abs_exec_root):
                # Add paths relative to exec_root.
    ...
    for p in os.environ.get("PATH", "").split(os.pathsep):
        try:
            if os.path.isabs(p) and p.startswith(abs_exec_root):
                # Add paths relative to exec_root.
                path.append(os.path.relpath(p))
            elif p.startswith(("/usr", "C:\\Program Files", "C:\\ProgramData",
                               "C:\\Windows")):
                # Keep system paths as is so python3 can be executed both
                # locally and remotely without issues.
                path.append(p)
        except ValueError:
            # Path and start are on different drives (Windows-specific).
            pass
    ...
    remote_env["PATH"] = os.pathsep.join(path)
    ...
    RBE_env_var_allowlist.append("PATH")
